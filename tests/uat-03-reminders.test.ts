...
describe('FR-08 / UAT-03 · reminder dispatch', () => {
  ...
  it('cancels rather than sends when the obligation is already paid', () => {
    const result = obligationsService.dispatch({
      obligationId: OBLIGATION_IDS.insuranceMarlin, // seeded as paid 1 Aug
      channel: 'email',
    ...
    const events = obligationsRepository.listReminders(OBLIGATION_IDS.insuranceMarlin);
    expect(events.some((event) => event.outcome === 'cancelled')).toBe(true);
  });

  it('suppresses reminders on a disputed obligation', () => {
    obligationsService.markDisputed(OBLIGATION_IDS.councilRates, USER_IDS.adam);
  ...
  it('builds a stable event-recipient-channel key that a retry recomputes', () => {
    ...
    expect(key).toBe(`${OBLIGATION_IDS.councilRates}:${USER_IDS.nadia}:email:2026-09-13`);
    expect(
      buildDispatchKey({
        obligationId: OBLIGATION_IDS.councilRates,
        recipientId: USER_IDS.nadia,
  ...
  it('sends nothing before an obligation enters its notice window', () => {
    // The Harlow insurance renewal falls due 14 Sep with a 5-day offset, so its
    ...
    expect(sentCount(OBLIGATION_IDS.insuranceHarlow)).toBeLessThanOrEqual(1);
    expect(beforeWindow.obligationId).toBe(OBLIGATION_IDS.insuranceHarlow);
    ...
    expect(selected).not.toContain(OBLIGATION_IDS.insuranceHarlow);
  });
